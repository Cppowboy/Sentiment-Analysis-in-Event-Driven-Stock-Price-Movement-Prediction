...
def calc_mid_long_return(ticker, date, delta, priceSet):
    ...
    try:
        return_self = log(priceSet[ticker]['Adj Close'][nextDate]) - log(priceSet[ticker]['Adj Close'][prevDate])
        return_sp500 = log(priceSet['^GSPC']['Adj Close'][nextDate]) - log(priceSet['^GSPC']['Adj Close'][prevDate])
